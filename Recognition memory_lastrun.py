...
expName = 'Recognition memory'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Student\\Desktop\\psycho2\\last\\Recognition memory_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('test_response') is None:
        # initialise test_response
        test_response = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='test_response',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "trial_words" ---
    word_stimulus = visual.TextStim(win=win, name='word_stimulus',
        text='',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.1, wrapWidth=None, ori=0.0, 
        color=[-1.0000, -1.0000, -1.0000], colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    test_response = keyboard.Keyboard(deviceName='test_response')
    ...
    WelcomeScreen = data.Routine(
        name='WelcomeScreen',
        components=[textWelcome, key_Welcome],
    )
    ...
    testInstructions = data.Routine(
        ...
        components=[textInstructions, key_instructions],
    )
    ...
    while continueRoutine:
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                win=win, 
                timers=[routineTimer, globalClock], 
                currentRoutine=testInstructions,
            )
            # skip the frame we paused on
            continue
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "trial_words" ---
        # create an object to store info about Routine trial_words
        trial_words = data.Routine(
            name='trial_words',
            components=[word_stimulus, test_response],
        )
        trial_words.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        word_stimulus.setText(study_word
        )
        # create starting attributes for test_response
        test_response.keys = []
        test_response.rt = []
        _test_response_allKeys = []
        # store start times for trial_words
        trial_words.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        trial_words.tStart = globalClock.getTime(format='float')
        trial_words.status = STARTED
        thisExp.addData('trial_words.started', trial_words.tStart)
        trial_words.maxDuration = None
        # keep track of which components have finished
        trial_wordsComponents = trial_words.components
        for thisComponent in trial_words.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "trial_words" ---
        trial_words.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # if trial has changed, end Routine now
            ...
            # *word_stimulus* updates
            
            # if word_stimulus is starting this frame...
            if word_stimulus.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                word_stimulus.frameNStart = frameN  # exact frame index
                word_stimulus.tStart = t  # local t and not account for scr refresh
                word_stimulus.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(word_stimulus, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'word_stimulus.started')
                # update status
                word_stimulus.status = STARTED
                word_stimulus.setAutoDraw(True)
            
            # if word_stimulus is active this frame...
            if word_stimulus.status == STARTED:
                # update params
                pass
            ...
            # if test_response is starting this frame...
            if test_response.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'test_response.started')
                # update status
            ...
            if test_response.status == STARTED and not waitOnFlip:
                theseKeys = test_response.getKeys(keyList=['y','n'], ignoreKeys=["escape"], waitRelease=False)
                _test_response_allKeys.extend(theseKeys)
                if len(_test_response_allKeys):
                    test_response.keys = _test_response_allKeys[-1].name  # just the last key pressed
                    test_response.rt = _test_response_allKeys[-1].rt
                    test_response.duration = _test_response_allKeys[-1].duration
                    # was this correct?
                    if (test_response.keys == str(correct_response)) or (test_response.keys == correct_response):
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                trial_words.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in trial_words.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "trial_words" ---
        for thisComponent in trial_words.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for trial_words
        trial_words.tStop = globalClock.getTime(format='float')
        trial_words.tStopRefresh = tThisFlipGlobal
        thisExp.addData('trial_words.stopped', trial_words.tStop)
        # check responses
        ...
        trials.addData('test_response.keys',test_response.keys)
        trials.addData('test_response.corr', test_response.corr)
        if test_response.keys != None:  # we had a response
            trials.addData('test_response.rt', test_response.rt)
            trials.addData('test_response.duration', test_response.duration)
        # the Routine "trial_words" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
